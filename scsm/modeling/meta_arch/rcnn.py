...
from scsm.modeling.meta_arch.ecea import ExtensibleTransformerEncoderLayer,build_positional_encoding,SinePositionalEncoding
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.BACKBONE.WITHECEA:
            ...
            out_channels = 256
            self.embed_dims = out_channels
            # Initialize lateral convolution layers
            norm = ""
            use_bias = norm == ""
            self.lateral_conv_in = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=use_bias)
            self.lateral_conv_out = nn.Conv2d(out_channels, in_channels, kernel_size=1, bias=use_bias)
            
            # More robust initialization
            nn.init.xavier_uniform_(self.lateral_conv_in.weight)
            if use_bias:
                nn.init.zeros_(self.lateral_conv_in.bias)
            nn.init.xavier_uniform_(self.lateral_conv_out.weight)
            if use_bias:
                nn.init.zeros_(self.lateral_conv_out.bias)
            
            # Positional encoding configuration

            self.positional_encoding = SinePositionalEncoding(
                num_feats=out_channels // 2,  # Half for x, half for y
                normalize=True
            )
            
            # extensible layer configuration
            self.encoder = nn.ModuleList([
                ExtensibleTransformerEncoderLayer(
                    embed_dim=out_channels,
                    num_heads=8,
                    num_points=4,
                    dropout=0.1
                ) for _ in range(4)  # 4 extensible layers
            ])
            ...
            self.level_embeds = nn.Parameter(
                torch.empty(self.num_feature_levels, self.embed_dims))
            nn.init.normal_(self.level_embeds)
            
             # Register helper buffer
            self.register_buffer('dummy', torch.zeros(1))  # Device placeholder
...
############################################################################################################################
        ...
        if self.SCSM:
            ...
            # extensiblepoint
            self.extensible_point = nn.Linear(self.embed_dims, 2)
            nn.init.xavier_uniform_(self.extensible_point.weight)
            nn.init.zeros_(self.extensible_point.bias)
            ...
            self.mamba_block = create_block(d_model=self.mamba_pooled)
...
############################################################################################################################
        ...
        self.ECEASA = cfg.MODEL.BACKBONE.WITHECEASA
        # if self.ECEASA:
        #     # add transofomer encoder
...
############################################################################################################################
        ...
        self.ECEASENet = cfg.MODEL.BACKBONE.WITHECEASENet
        # if self.ECEASENet:
        #     # add transofomer encoder
...
###########################################################################################################################
        """
            ...
            - DEFRCN+SENet
        """
        from scsm.modeling.meta_arch.myTransformer import Defrcn_SE_Block
        self.SENet = cfg.MODEL.BACKBONE.SENET
        if self.SENet:     
            self.dese_block = Defrcn_SE_Block(inchannel=1024, ratio=16)
...
###########################################################################################################################
        """
            ...
            - CBAM
        """
        from scsm.modeling.meta_arch.myTransformer import CBAM
        self.CBAM = cfg.MODEL.BACKBONE.CBAM
        if self.CBAM:     
            self.de_cbam = CBAM(in_planes=1024)
...
###########################################################################################################################
        """
            ...
            - CBAM
        """
        from scsm.modeling.meta_arch.myTransformer import ECA
        self.ECA = cfg.MODEL.BACKBONE.ECA
        if self.ECA:     
            self.de_eca = ECA(in_channels=1024)
...
############################################################################################################################
    ...
    def init_weights(self):
        ...
        # 特殊模块初始化
        for m in self.modules():
            if isinstance(m, ExtensibleTransformerEncoderLayer):
    ...
    def _get_reference_points(self, spatial_shapes, valid_ratios, device):
        """生成归一化的参考点坐标"""
        ...
        for lvl, (H, W) in enumerate(spatial_shapes):
            ...
            ref_y = ref_y.reshape(-1) / (valid_ratios[:, lvl, 1] * H).view(-1, 1)
            ref_x = ref_x.reshape(-1) / (valid_ratios[:, lvl, 0] * W).view(-1, 1)
            reference_points_list.append(torch.stack((ref_x, ref_y), -1))
        return torch.cat(reference_points_list, dim=1)

    def _run_encoder(self, src, pos, mask, spatial_shapes, level_start_index, reference_points):
        """封装编码器处理流程"""
        src = src.permute(2, 0, 1)  # [Σ(H*W), B, C]
        pos = pos.permute(1, 0, 2)  # [Σ(H*W), B, C]
        ...
        #     )
        reference_points = reference_points.permute(1, 0, 2)  # [Σ(H*W), B, 2]
        for layer in self.encoder:
            src = layer(
                src,
                pos,
        ...
        return src.permute(1, 2, 0)  # [B, C, Σ(H*W)]

    def _reshape_output(self, memory, spatial_shapes, batch_size):
        """将展平的特征重组为多尺度特征图"""
        chunks = memory.split([(H*W).item() for H, W in spatial_shapes], dim=-1)
        return [
            chunk.view(batch_size, -1, H, W)
            for chunk, (H, W) in zip(chunks, spatial_shapes)
        ]
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None, is_attack = False, iter = None):
        ...
        if self.cfg.MODEL.BACKBONE.WITHECEA:
            ...
            # Dynamically compute valid region masks
            img_masks = torch.ones((batch_size, *mlvl_feats[0].shape[-2:]), 
                                dtype=torch.bool, device=device)
            for img_id in range(batch_size):
                h, w = mlvl_feats[0].shape[-2:]
                img_masks[img_id, :h, :w] = False
            ...
            # 4. Flatten features
            feat_flatten = torch.cat([f.flatten(2) for f in mlvl_feats], dim=2)  # [B, C, Σ(H*W)]
            mask_flatten = torch.cat([m.flatten(1) for m in mlvl_masks], dim=1)  # [B, Σ(H*W)]
            
            # 5. Position encoding processing
            spatial_shapes = torch.tensor(
                [f.shape[-2:] for f in mlvl_feats], 
                device=device
            )
            ...
            # 7. Encoder input preparation
            query_pos = torch.cat([
                pe.flatten(2).permute(0, 2, 1) + self.level_embeds[i].view(1, 1, -1)
                for i, pe in enumerate(mlvl_pos_embeds)
            ], dim=1)  # [B, Σ(H*W), C]
            
            # 8. Encoder processing            
            memory = self._run_encoder(
                feat_flatten,
                query_pos,
                mask_flatten,
                spatial_shapes,
            ...
            # 9. Feature reorganization
            out_results = self._reshape_output(
                memory,
                spatial_shapes,
                batch_size
            )
            ...
            # 10. Feature fusion
            for key, res in zip(features.keys(), out_results):
                features[key] = features[key] * 0.6 + self.lateral_conv_out(res) * 0.4 
...
##############################################################################################################
        ...
        if self.SCSM:
            ...
            ecea_feature = self._run_encoder(
                feat_flatten,
                query_pos,
                mask_flatten,
                spatial_shapes,
            ...
            B, C, H, W = features["res4"].shape
            res_feature = features["res4"].view(B, C, H * W) 
            if res_feature.shape[1] != pooled_C:
                res_feature = F.interpolate(res_feature.permute(0, 2, 1), size=pooled_C, mode='nearest')
            else:
                res_feature = res_feature.permute(0, 2, 1)
            csm_feature, _ = self.mamba_block(res_feature)
            csm_feature = csm_feature.permute(0, 2, 1)
            
            
            # ecea fused csm
            fused_feature = ecea_feature + csm_feature  
            ...
            for key, res in zip(features.keys(), out_results):
                features[key] = features[key] + self.lateral_conv_out(res)
...
###################################################################################################################
        ...
        if self.SENet:
            ...
            se_res4 = self.dese_block(res4_feature)
            features["res4"] = se_res4 + features["res4"]
...
###################################################################################################################
        ...
        if self.CBAM:
            res4_feature = features["res4"]
            cbam_res4 = self.de_cbam(res4_feature)
            features["res4"] = cbam_res4 + res4_feature
...
###################################################################################################################
        ...
        if self.ECA:
            res4_feature = features["res4"]
            eca_res4 = self.de_eca(res4_feature)
            features["res4"] = eca_res4 + res4_feature
