...
from scsm.data import *
from scsm.modeling import build_model
from scsm.engine.hooks import EvalHookSCSM
from scsm.checkpoint import DetectionCheckpointer
from scsm.solver import build_lr_scheduler, build_optimizer
from scsm.evaluation import DatasetEvaluator, inference_on_dataset, print_csv_format, verify_results
from scsm.dataloader import MetadataCatalog, build_detection_test_loader, build_detection_train_loader
...
def default_argument_parser():
    ...
    """
    parser = argparse.ArgumentParser(description="SCSM Training")
    ...
    parser.add_argument("--data_aug", default="None", help="data augmentation type", type=str, choices=["None", "Cutout", "Mixup", "CutMix"])
    parser.add_argument("--feature_aug", default="None", help="feature augmentation type", type=str, choices=["None", "Cutout", "Mixup", "CutMix"])
    parser.add_argument("--output_dir", default="None",help="output directory", type=str)
    # PyTorch still may leave orphan processes in multi-gpu training.
    # Therefore we use a deterministic way to obtain port,
    # so that users are aware of orphan processes by seeing the port occupied.
    #port = 2 ** 15 + 2 ** 14 + hash(os.getuid()) % 2 ** 14
    parser.add_argument("--dist-url", default="tcp://127.0.0.1:{}".format(24900))
...
def default_setup(cfg, args):
    ...
    setup_logger(output_dir, distributed_rank=rank, name="scsm")
